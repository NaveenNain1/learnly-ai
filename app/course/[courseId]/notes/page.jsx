...
const ViewNotes = () => {
  ...
  const parseNoteContent = (noteString) => {
    ...
    const enhanceContent = (html) => {
      if (!showCodeBlocks || typeof html !== "string") return html;
    ...
    try {
      ...
      if (parsedNote && parsedNote.topics && Array.isArray(parsedNote.topics)) {
        return (
        ...
        return (
          <div
            dangerouslySetInnerHTML={{
              __html: enhanceContent(parsedNote?.content || noteString)
            }}
      ...
      // If noteString is direct HTML (e.g. <div>...</div>) or non-JSON text, render directly as HTML
      return (
        <div
          dangerouslySetInnerHTML={{
            __html: enhanceContent(noteString)
          }}
          className="prose max-w-3xl mx-auto"
        />
      );
